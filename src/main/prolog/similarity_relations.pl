...
% Trace is a trace generated by the symbolic execution of TP starting from Caller
trace(TP,Trace) :-
  test_entry_point(TP,Caller),
...
% SEMANTICS: test_entry_point(TP,Caller) holds if
% Caller is the first invocation performed in TP
test_entry_point(TP,Caller) :-
...
generate_and_assert_endpoints(miseq) :-
  ...
  findall(
    (TP,ESeq),
    ( member(TP,TPs), test_entry_point(TP,Caller),
